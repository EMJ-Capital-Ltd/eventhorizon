 ...
 * Detects momentum shifts and inflection points
 *
 * @param points Signal data points (must be sorted by date ascending)
 * @returns Acceleration in decimal form (e.g., 0.001 = +0.1pp/d²)
 */
export function calculateAcceleration(points: SignalPoint[]): number {
  if (points.length < 7) return 0;

  // Calculate velocity at two different time points
  const recentVelocity = calculateVelocity(points.slice(-4));
  const priorVelocity = calculateVelocity(points.slice(-7, -3));
 ...
 * Detects sudden shifts in market conviction
 *
 * @param points Signal data points (must be sorted by date ascending)
 * @returns Jerk in decimal form
 */
export function calculateJerk(points: SignalPoint[]): number {
  if (points.length < 10) return 0;

  // Calculate acceleration at two different time points
  const recentAccel = calculateAcceleration(points.slice(-7));
  const priorAccel = calculateAcceleration(points.slice(-10, -3));

  // Jerk = change in acceleration over time
  const jerk = recentAccel - priorAccel;

  return jerk;
}

/**
 * Detect belief "flips" - inflection points where market conviction changes direction
 * Uses 2nd derivative (acceleration) to identify momentum reversals
 *
 * @param points Signal data points (must be sorted by date ascending)
 * @returns Object with flip detection and type
 */
export function detectBeliefFlip(points: SignalPoint[]): {
  hasFlip: boolean;
  flipType: 'bullish' | 'bearish' | 'none';
  strength: number;
} {
  if (points.length < 7) {
    return { hasFlip: false, flipType: 'none', strength: 0 };
  }
  ...
  // A flip occurs when:
  // 1. Velocity is changing direction (acceleration has opposite sign)
  // 2. The magnitude of acceleration is significant

  const threshold = 0.002; // ~0.2pp/d² threshold for significant acceleration

  if (Math.abs(acceleration) < threshold) {
    return { hasFlip: false, flipType: 'none', strength: 0 };
  }

  // Bullish flip: negative velocity becoming less negative (or positive becoming more positive)
  // i.e., acceleration is positive
  ...
  // Bearish flip: positive velocity becoming less positive (or negative becoming more negative)
  // i.e., acceleration is negative
  if (acceleration < -threshold) {
    return {
      hasFlip: true,
      flipType: 'bearish',
