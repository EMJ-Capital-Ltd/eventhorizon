/**
 * Seed data for EventHorizon dashboard
 * Real events with simulated probability trajectories
 */
...
export interface Event {
  id: string;
  title: string;
  category: 'regulatory' | 'macro' | 'crypto' | 'geopolitical';
  description: string;
...
// Generate realistic probability trajectory
...
// Generate trajectory with a sharp move (regime change)
function generateSharpMove(
  startProb: number,
  midProb: number,
  endProb: number,
  sharpPoint: number = 0.7, // when the sharp move happens (0-1)
  hours: number = 168
): ProbabilityPoint[] {
  const points: ProbabilityPoint[] = [];
  const now = Date.now();
  ...
  for (let i = hours; i >= 0; i--) {
    ...
    if (i > sharpHour) {
      // Before sharp move - drift toward midProb
      const progress = (hours - i) / (hours - sharpHour);
      prob = startProb + (midProb - startProb) * progress;
    } else {
      // After sharp move - quick transition to endProb
...
// Calculate aggregate stress indicator
export function calculateStressIndex(events: Event[]): number {
  ...
  // Stress = weighted combination
  const regimeStress = (volatileCount * 0.4 + transitioningCount * 0.2) / events.length;
  const velocityStress = Math.min(1, avgAbsVelocity / 0.02); // Normalize to 0-1
